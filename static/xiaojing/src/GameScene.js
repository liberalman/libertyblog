...
var StartUI = cc.Layer.extend({
    ...
    onEnter : function () { // onEnter方法是场景初始化完成即将展示的消息回调，在onEnter中必须调用this._super();来确保Scene被正确的初始化。
        this._super();
        ...
        cc.eventManager.addListener({
            ...
            onTouchesEnded: function (touches, event) {
                ...
                if (pos.y < cc.winSize.height/3) {
                    layers.game.initGame(); // 初始化游戏
                    layers.startUI.removeFromParent(); //移除开始界面
                }
            }
        }, this);
    }
...
var ResultUI = cc.Layer.extend({
    ...
    onEnter : function () {
        ...
        if (this.win) {
            ...
            var label = new cc.LabelTTF("我为晓静点了" + step+ "根蜡烛\n祝福晓静永远" + (step + 10) + "岁\n打败"+percent+"%朋友圈的人\n渣渣们来挑战我啊！", "宋体", 20);
            label.x = w/2;
            label.y = h/4;
...
var AnimationLayer = cc.Layer.extend({
    ...
    init:function () {
        ...
        // create sprite sheet
        cc.spriteFrameCache.addSpriteFrames(res.npc1_plist);
        this.spriteSheet = cc.SpriteBatchNode.create(res.npc1_png);
        ...
        this.sprite.attr({x:cc.winSize.width/10*8, y:cc.winSize.height/10*7});
...
var GameScene = cc.Scene.extend({
    onEnter : function () {
        ...
        layers.bg1 = new AnimationLayer();
        this.addChild(layers.bg1);
